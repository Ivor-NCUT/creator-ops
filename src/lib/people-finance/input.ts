...
const month = { year: z.coerce.number().int().min(2000).max(2200), month: z.coerce.number().int().min(1).max(12) };
...
export const payrollInput = z.object({ employeeId: z.string().min(1), ...month, currency: z.string().trim().length(3).transform((v) => v.toUpperCase()), baseSalary: money, performance: money, commission: money, allowance: money, penalties: money, note: optionalText(500) });
export const majorErrorInput = z.object({ employeeId: z.string().min(1), occurredOn: z.coerce.date(), title: z.string().trim().min(2).max(160), detail: optionalText(2000), penaltyAmount: money });
