...
- [直播电商与客诉](docs/architecture/live-commerce.md)
- [人员、财务与 CRM](docs/architecture/people-finance-crm.md)
- [经营看板与固定自动化](docs/architecture/analytics-automations.md)

## 本地启动

```bash
cp .env.example .env
# 把 BETTER_AUTH_SECRET 和 AUTOMATION_SECRET 替换为两份独立随机值
pnpm install
...
自动化调度入口为 `POST /api/internal/automations/tick`，请求头使用 `Authorization: Bearer $AUTOMATION_SECRET`。部署平台应每小时调用一次；重复调用不会重复生成通知。
...
正在开发首个可部署版本。仓库不包含原始飞书数据，只提供明显标注的虚构示例数据。
